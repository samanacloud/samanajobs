...
function getPositionAppliedFor($candidate) {
    foreach ($candidate as $key => $value) {
        if (strpos($value, 'L1') !== false || strpos($value, 'L2') !== false || strpos($value, 'L3') !== false) {
            return $value;
        }
    }
    return '';
...
<div id="candidate-list" class="row">
	<h4>Candidates Registered through Google Forms</h4>
    <div class="col-md-12">
...
<div id="candidate-details" style="display:none;">
    ...
    <form id="enrollmentForm" action="user_enrollment.php" method="POST">
        ...
        <button type="submit" class="btn btn-primary">Enroll Candidate</button>
    </form>
...
<script>
    ...
    function showDetails(index) {
        ...
        Object.entries(candidate).forEach(([key, value], idx) => {
            ...
            keyDiv.classList.add('col-md-6', 'font-weight-bold');
            keyDiv.textContent = cleanKey;
            ...
            valueDiv.innerHTML = parsedValue;
            row.appendChild(keyDiv);
            row.appendChild(valueDiv);

            // Add rows to the appropriate column
            if (idx % 2 === 0) {
        ...
        document.getElementById('email').value = candidate['Email Address'] || '';

        document.getElementById('enrollmentForm').style.display = 'block';
    }

    function closeDetails() {
        document.getElementById('candidate-details').style.display = 'none';

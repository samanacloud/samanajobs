    ...
    <div class="row">
        <div class="col-md-6">
            <div class="form-container">
                ...
                <p><strong>Phone Number:</strong>     <a href="https://web.whatsapp.com/send?phone=<?php echo htmlspecialchars($candidateInfo['phone_number']); ?>&text=<?php echo urlencode($message); ?>" target="_blank">        <?php echo htmlspecialchars($candidateInfo['phone_number']); ?>    </a></p>
				...
				<div class="btn-group">
					...
					<form action="candidate_skillsets.php" method="POST" class="d-flex">
					  ...
					  <button id="scheduleInterviewButton" class="btn btn-primary">Schedule Interview</button>
				</div>	
            </div>
...
document.getElementById('scheduleInterviewButton').addEventListener('click', function() {
    ...
    createCalendarInvitation(
        ...
        "recruiting@samanagroup.co",
        "<?php echo $candidateCV; ?>",
        "<?php echo $first_interview_data['process']; ?>",
        "<?php echo $_SESSION['Name']; ?>"
    );
});

function createCalendarInvitation(candidateEmail, sessionEmail, recruitingEmail, candidateCV, process, recruiterName) {
    const candidateProfileURL = `https://jobs.samana.cloud/admin/candidate_profile.php?email=${candidateEmail}`;
    const event = {
        action: 'TEMPLATE',
        text: process,
        details: `Hello ${candidateEmail},\n\nCV: <a href="${candidateCV}">Link to CV</a>\nCandidate Profile: <a href="${candidateProfileURL}">Candidate Profile</a>\n\nThank you for your application for the ${process} position. As discussed in our recent conversation, your skills and experience align well with the requirements of this role. The next step in our recruitment process is a video interview with the rest of the team.\n\nPlease feel free to contact us if you have any questions.\n\nBest regards,\n\n${recruiterName}\nRecruiting Lead\nSamana Group LLC`,
        location: '',
        trp: 'false',
        sprop: `website:${candidateCV}`,
        spropname: 'Candidate CV',
        add: `${candidateEmail},${sessionEmail},${recruitingEmail}`
    };

    const baseUrl = 'https://www.google.com/calendar/render';
    const queryString = Object.keys(event)
        .map(key => `${encodeURIComponent(key)}=${encodeURIComponent(event[key])}`)
        .join('&');
